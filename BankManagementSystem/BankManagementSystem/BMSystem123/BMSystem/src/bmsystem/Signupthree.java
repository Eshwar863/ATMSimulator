...
public class Signupthree extends JFrame implements ActionListener{
    ...
    app= new JLabel("Page 3: Account Details "+dbformno);
    ...
    add(cancelb);
        ...
        String dbformno2 = dbformno;
          ...
          String dbcardnor = "" + Math.abs((random.nextLong() % 90000000)+50000000000L);
         ...
         try{
             ...
             String query = "UPDATE commontable SET  accountType= ?, cardNo= ?, pinNo= ?, facility= ? WHERE formNo = " + dbformno + ";";
             PreparedStatement p = c.prepareStatement(query);
             p.setString(1, dbAccountType);
             p.setString(2, dbcardnor);
             p.setString(3, dbpinnor);
             p.setString(4, dbfacility);

             int i = p.executeUpdate();
             DAO dao = new DAO();
             dao.insertBalance(dbcardnor);
             if (i > 0)
